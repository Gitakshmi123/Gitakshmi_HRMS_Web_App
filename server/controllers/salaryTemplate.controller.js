...
exports.calculateBreakup = async (req, res) => {
    try {
        const { annualCTC, minWageAmount: manualMinWage, employeeCategory, state, city, earnings = [], deductions = [], benefits = [] } = req.body;

        if (!annualCTC || isNaN(Number(annualCTC)) || Number(annualCTC) <= 0) {
        ...
        // 1. Resolve Minimum Wage if not manually provided
        let minWageAmount = Number(manualMinWage || 0);
        if (minWageAmount <= 0 && state && employeeCategory) {
            try {
                const MinimumWage = req.tenantDB.model('MinimumWage');
                const mwDoc = await MinimumWage.findOne({ 
                    tenantId, 
                    state: { $regex: new RegExp(`^${state}$`, 'i') },
                    category: { $regex: new RegExp(`^${employeeCategory}$`, 'i') }
                });
                if (mwDoc) minWageAmount = mwDoc.monthlyAmount;
            } catch (mwErr) {
                console.warn('[CALCULATE_BREAKUP] MW fetch failed:', mwErr.message);
            }
        }

        // 2. Resolve Statutory Rules (PT, PF, ESIC) for the preview
