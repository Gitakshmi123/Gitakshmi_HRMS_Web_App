...
export default function RegularizationRequest() {
  ...
  const handleFilter = () => {
    ...
    else if (activeTab === 'Cancelled') filtered = filtered.filter(item => item.status === 'Cancelled' || item.status === 'CANCELLED');
    ...
    // Date filtering can be added here if dates are formatted properly
  ...
  const columns = [
    ...
    { title: 'Current Status', dataIndex: 'status', width: 100, render: text => <Text type="secondary">{text}</Text> },
