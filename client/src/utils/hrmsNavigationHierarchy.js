...
export function getSectionTabs(pathPrefix, icons = {}) {
  ...
  return [
    ...
    {
      match: ['/payroll', '/salary-structure', '/payslip-templates', '/minimum-wage', '/payroll-rules', '/payroll-reports', '/salary-assignment-excel'],
      tabs: [
        { label: 'Payroll Dashboard', to: buildPath(pathPrefix, 'payroll/dashboard'), icon: icon('dashboard'), permission: 'payroll.stats' },
        { label: 'Payroll Process', to: buildPath(pathPrefix, 'payroll/process'), icon: icon('settings'), permission: 'payroll.process' },
        { label: 'Employee Payroll', to: buildPath(pathPrefix, 'payroll/employee-payroll'), icon: icon('employees'), permission: 'payroll.stats' },
        { label: 'Payslip', to: buildPath(pathPrefix, 'payroll/payslip-view'), icon: icon('file'), permission: 'payroll.payslips' },
        { label: 'Arrear / Onetime Payment', to: buildPath(pathPrefix, 'payroll/arrears'), icon: icon('settings'), permission: 'payroll.process' },
        { label: 'Reimbursement', to: buildPath(pathPrefix, 'payroll/reimbursements'), icon: icon('settings'), permission: 'payroll.process' },
        { label: 'Loan & Advance', to: buildPath(pathPrefix, 'payroll/loans'), icon: icon('settings'), permission: 'payroll.process' },
        { label: 'Deduction Entry', to: buildPath(pathPrefix, 'payroll/deduction-entry'), icon: icon('settings'), permission: 'payroll.salary' },
        { label: 'TDS Declaration', to: buildPath(pathPrefix, 'payroll/tds-declaration'), icon: icon('settings'), permission: 'payroll.process' },
        { label: 'Other Earnings', to: buildPath(pathPrefix, 'payroll/other-earnings'), icon: icon('settings'), permission: 'payroll.process' },
        { label: 'Payroll Reports', to: buildPath(pathPrefix, 'payroll/reports'), icon: icon('history'), permission: 'payroll.stats' },
        { label: 'Form 16', to: buildPath(pathPrefix, 'payroll/form16'), icon: icon('file'), permission: 'payroll.stats' },
        { label: 'Salary Revision', to: buildPath(pathPrefix, 'payroll/salary-revision'), icon: icon('settings'), permission: 'payroll.compensation' },
      ],
    },
    {
