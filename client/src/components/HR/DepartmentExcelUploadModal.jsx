...
export default function DepartmentExcelUploadModal({ isOpen, onClose, onSuccess }) {
  ...
  const validateRow = (row, rowIndex) => {
    ...
    const name = getFieldValue(row, ['departmentname', 'name', 'deptname']);
    const code = getFieldValue(row, ['departmentcode', 'code', 'deptcode']);

    if (!name) errors.push('Department Name is missing (Required)');
    else if (name.length < 2) warnings.push('Department Name should be at least 2 characters');

    if (!code) errors.push('Department Code is missing (Required)');
  ...
  const validateFileStructure = (data) => {
    ...
    const requiredChecks = [
      { display: 'Department Name', patterns: ['departmentname', 'name', 'deptname'] },
      { display: 'Department Code', patterns: ['departmentcode', 'code', 'deptcode'] }
    ];
  ...
  const handleDownloadTemplate = () => {
    try {
      // Generate template directly in frontend
      const templateData = [
        {
      ...
      XLSX.writeFile(wb, `Department_Template_${new Date().getTime()}.xlsx`);
    } catch (err) {
    ...
    reader.onload = (e) => {
      try {
        ...
        if (jsonData.length === 0) {
          setUploadErrors(['Excel file is empty. Please add department records.']);
          setUploadedFile(null);
          return;
        }
  ...
  const handleSubmitUpload = async () => {
    ...
    try {
      ...
      const res = await api.post('/hr/departments/bulk-upload', payload);

      if (res.data) {
        if (res.data.success === false) {
          const errorMessage = res.data.message || 'Failed to upload departments. Please try again.';
      ...
      if (err.response?.data?.errors && Array.isArray(err.response.data.errors)) {
        ...
        setUploadErrors([errorMessage || 'Failed to upload departments. Please try again.']);
      }
      showToast('error', 'Upload Failed', errorMessage || 'Failed to upload departments.');
    } finally {
      setUploading(false);
    }
  };
      ...
      <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-hidden flex flex-col">
        ...
        <div className="sticky top-0 z-10 bg-gradient-to-r from-blue-600 to-blue-700 dark:from-blue-800 dark:to-blue-900 p-4 border-b border-blue-500 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-lg font-bold text-white">Bulk Add Departments</h3>
              <p className="text-xs text-blue-100 mt-0.5">Import department data from Excel</p>
            </div>
          </div>
          <button
            onClick={onClose}
          ...
          <div className="bg-emerald-50 border border-emerald-200 rounded-xl p-3.5 flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-slate-600 mt-0.5">Includes required columns: Department Name, Department Code</p>
              </div>
            </div>
            <button
            ...
            {!uploadedFile ? (
              <div>
                <input
                  ...
                  id="dept-file-upload"
                />
                <label htmlFor="dept-file-upload" className="block cursor-pointer">
