...
export default function GradeExcelUploadModal({ isOpen, onClose, onSuccess }) {
  ...
  const validateRow = (row, rowIndex) => {
    ...
    const name = getFieldValue(row, ['gradename', 'name', 'grade']);
    const code = getFieldValue(row, ['gradecode', 'code', 'shortcode']);
    const level = getFieldValue(row, ['hierarchylevel', 'level', 'gradelevel', 'hierarchy']);

    if (!name) {
      errors.push('Grade Name is missing (Required)');
    } else if (name.length < 2) {
      warnings.push('Grade Name should be at least 2 characters');
    }

    if (!code) {
      errors.push('Grade Code is missing (Required)');
    }

    if (!level) {
      errors.push('Hierarchy Level is missing (Required)');
    } else {
      const parsedLevel = parseInt(level, 10);
      if (isNaN(parsedLevel) || parsedLevel < 1) {
        errors.push('Hierarchy Level must be a valid positive integer');
      }
    }
  ...
  const validateFileStructure = (data) => {
    ...
    const requiredChecks = [
      { display: 'Grade Name', patterns: ['gradename', 'name', 'grade'] },
      { display: 'Grade Code', patterns: ['gradecode', 'code', 'shortcode'] },
      { display: 'Hierarchy Level', patterns: ['hierarchylevel', 'level', 'gradelevel', 'hierarchy'] }
    ];
  ...
  const handleDownloadTemplate = () => {
    try {
      const templateData = [
        {
          'Grade Name': 'Senior Director',
          'Grade Code': 'DIR',
          'Hierarchy Level': '9',
          'Description': 'Executive leadership role'
        },
        {
          'Grade Name': 'Software Engineer',
          'Grade Code': 'SE',
          'Hierarchy Level': '2',
          'Description': 'Individual contributor role'
        }
      ];
      ...
      XLSX.writeFile(wb, `Grade_Template_${new Date().getTime()}.xlsx`);
    } catch (err) {
    ...
    reader.onload = (e) => {
      try {
        ...
        if (jsonData.length === 0) {
          setUploadErrors(['Excel file is empty. Please add grade records.']);
          setUploadedFile(null);
          return;
        }
  ...
  const handleSubmitUpload = async () => {
    ...
    try {
      ...
      const res = await api.post('/grades/bulk-upload', payload);

      if (res.data) {
        if (res.data.success === false) {
          const errorMessage = res.data.message || 'Failed to upload grades. Please try again.';
      ...
      if (err.response?.data?.errors && Array.isArray(err.response.data.errors)) {
        ...
        setUploadErrors([errorMessage || 'Failed to upload grades. Please try again.']);
      }
      showToast('error', 'Upload Failed', errorMessage || 'Failed to upload grades.');
    } finally {
      setUploading(false);
    }
  };
      ...
      <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-hidden flex flex-col">
        ...
        <div className="sticky top-0 z-10 bg-gradient-to-r from-blue-600 to-blue-700 dark:from-blue-800 dark:to-blue-900 p-4 border-b border-blue-500 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-lg font-bold text-white">Bulk Add Grades</h3>
              <p className="text-xs text-blue-100 mt-0.5">Import grade data from Excel</p>
            </div>
          </div>
          <button
            onClick={onClose}
          ...
          <div className="bg-emerald-50 border border-emerald-200 rounded-xl p-3.5 flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-slate-600 mt-0.5">Includes required columns: Grade Name, Grade Code, Hierarchy Level</p>
              </div>
            </div>
            <button
          ...
          <div>
            {!uploadedFile ? (
              <div>
                ...
                />
                <label htmlFor="grade-file-upload" className="block cursor-pointer">
