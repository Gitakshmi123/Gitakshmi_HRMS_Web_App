        ...
        <div className="flex flex-col border-b border-gray-200 bg-white sticky top-0 z-30 shadow-sm print:hidden">
            <div className="flex flex-wrap items-center gap-1 p-2">
                ...
                <div className="flex items-center space-x-0.5 border-r border-gray-300 pr-2 mr-1">
                    <button onClick={() => editor.chain().focus().toggleBold().run()} className={`p-1.5 rounded transition-colors ${editor.isActive('bold') ? 'bg-indigo-50 text-indigo-600' : 'hover:bg-slate-50'}`}><Bold size={18} /></button>
                    <button onClick={() => editor.chain().focus().toggleItalic().run()} className={`p-1.5 rounded transition-colors ${editor.isActive('italic') ? 'bg-indigo-50 text-indigo-600' : 'hover:bg-slate-50'}`}><Italic size={18} /></button>
                    <button onClick={() => editor.chain().focus().toggleUnderline().run()} className={`p-1.5 rounded transition-colors ${editor.isActive('underline') ? 'bg-indigo-50 text-indigo-600' : 'hover:bg-slate-50'}`}><UnderlineIcon size={18} /></button>
                </div>

                {/* Lists & Align */}
                <div className="flex items-center space-x-0.5 border-r border-gray-300 pr-2 mr-1">
                    <button onClick={() => editor.chain().focus().toggleBulletList().run()} className={`p-1.5 rounded transition-colors ${editor.isActive('bulletList') ? 'bg-indigo-50 text-indigo-600' : 'hover:bg-slate-50'}`}><List size={18} /></button>
                    <button onClick={() => editor.chain().focus().toggleOrderedList().run()} className={`p-1.5 rounded transition-colors ${editor.isActive('orderedList') ? 'bg-indigo-50 text-indigo-600' : 'hover:bg-slate-50'}`}><ListOrdered size={18} /></button>
                    ...
                    }} className="p-1.5 rounded hover:bg-slate-50" title="Outdent (Shift+Tab)">
                        <Outdent size={18} />
                    </button>
                    <button onClick={() => {
                        if (editor.isActive('listItem')) editor.chain().focus().sinkListItem('listItem').run();
                        else editor.chain().focus().indent().run();
                    }} className="p-1.5 rounded hover:bg-slate-50" title="Indent (Tab)">
                        <Indent size={18} />
                    </button>

                    <div className="w-px h-6 bg-gray-300 mx-1"></div>

                    <button onClick={() => editor.chain().focus().setTextAlign('left').run()} className={`p-1.5 rounded transition-colors ${editor.isActive({ textAlign: 'left' }) ? 'bg-indigo-50 text-indigo-600' : 'hover:bg-slate-50'}`}><AlignLeft size={18} /></button>
                    <button onClick={() => editor.chain().focus().setTextAlign('center').run()} className={`p-1.5 rounded transition-colors ${editor.isActive({ textAlign: 'center' }) ? 'bg-indigo-50 text-indigo-600' : 'hover:bg-slate-50'}`}><AlignCenter size={18} /></button>
                    <button onClick={() => editor.chain().focus().setTextAlign('justify').run()} className={`p-1.5 rounded transition-colors ${editor.isActive({ textAlign: 'justify' }) ? 'bg-indigo-50 text-indigo-600' : 'hover:bg-slate-50'}`}><AlignJustify size={18} /></button>
                </div>
                ...
                <div className="flex items-center space-x-1">
                    ...
                    <div className="relative">
                        <button onClick={() => setShowSpacingMenu(!showSpacingMenu)} className={`p-1.5 rounded transition-colors ${showSpacingMenu ? 'bg-indigo-50 text-indigo-600' : 'hover:bg-slate-50'}`} title="Spacing"><ArrowUpWideNarrow size={18} /></button>
                        {showSpacingMenu && (
                    ...
                    <button onClick={onInsertImage} className="p-1.5 rounded hover:bg-slate-50 transition-colors"><ImageIcon size={18} /></button>
                    <button onClick={() => editor.chain().focus().setHorizontalRule().run()} className="p-1.5 rounded hover:bg-slate-50 transition-colors" title="Insert Horizontal Line"><Minus size={18} /></button>

                    <div className="relative">
                        <button onClick={() => setShowTableMenu(!showTableMenu)} className={`p-1.5 rounded transition-colors ${showTableMenu ? 'bg-indigo-50 text-indigo-600' : 'hover:bg-slate-50'}`}><TableIcon size={18} /></button>
                        {showTableMenu && (
                ...
                {!isLetterPad && (
                    <div className="ml-auto flex items-center gap-2">
                        <button onClick={() => setShowHeader(!showHeader)} className="text-[10px] font-bold uppercase tracking-widest flex items-center gap-2 px-3 py-1.5 rounded-xl bg-slate-50 hover:bg-slate-100 transition-all text-slate-500">
                            {showHeader ? <CheckSquare size={14} className="text-indigo-600" /> : <Square size={14} />} Header
                        </button>
                        <button onClick={() => setShowFooter(!showFooter)} className="text-[10px] font-bold uppercase tracking-widest flex items-center gap-2 px-3 py-1.5 rounded-xl bg-slate-50 hover:bg-slate-100 transition-all text-slate-500">
                            {showFooter ? <CheckSquare size={14} className="text-indigo-600" /> : <Square size={14} />} Footer
                        </button>
                    </div>
                )}
            </div>
            ...
            <div className="flex items-center gap-2 px-3 py-1.5 bg-gray-50 border-t border-gray-200 text-xs overflow-x-auto">
                ...
                {[
                    ...
                    <button key={p.id} onClick={() => editor.chain().focus().insertPlaceholder({ id: p.id, label: p.l }).run()} className="px-3 py-1 bg-white border border-slate-200 rounded-lg hover:bg-indigo-50 hover:border-indigo-100 transition-all font-mono text-[9px] text-indigo-600 font-bold">
                        {`{${p.l}}`}
                    </button>
                ))}
            </div>
...
const OfferLetterEditor = ({
    ...
    return (
        <div className="flex flex-col h-full bg-slate-100 items-center overflow-auto py-8">
            ...
            <style>{`
                ...
                .a4-page {
                    ...
                    ${isLetterPad && backgroundUrl ? `
                        background-image: url('${backgroundUrl.startsWith('http') ? backgroundUrl : API_ROOT + (backgroundUrl.startsWith('/') ? '' : '/') + backgroundUrl}');
                ...
                .zone-resizer-handle::before { content: ""; width: 40px; height: 4px; background: #4F46E5; border-radius: 2px; }
            ...
            <div className="flex justify-end gap-4 print:hidden w-[210mm] mt-8 pb-10">
                <button onClick={handleSave} className="px-10 py-4 bg-[#4F46E5] hover:bg-[#0D9488] text-white rounded-2xl font-bold text-xs uppercase tracking-[0.2em] transition-all flex items-center gap-3 shadow-lg shadow-indigo-500/20 active:scale-95">
                    <Save size={18} /> Save & Synchronize Template
                </button>
            </div>
        </div>
    );
