...
<div class="bg-white p-8 max-w-4xl mx-auto rounded-lg">
  <div class="flex justify-between items-start mb-8 pb-6 border-b-2 border-blue-600">
    <div>
      ...
      <p class="text-sm text-slate-600">${companyInfo?.tagline || ''}</p>
    </div>
  </div>
  ...
  <ol class="list-decimal ml-6 space-y-2 text-sm">
    ...
    <li><b>Probation:</b> ${offerData?.probationPeriod || '_________'}</li>
  ...
  <div class="mt-12 border-t pt-4">
    ...
    <p class="mt-6 font-semibold">${applicant?.name?.toUpperCase()}</p>
        ...
        // Custom Replacements (Support Dynamic Custom Data + Built-in Mappings)
        let customReplacements = {};
        if (offerData?.customData) {
           customReplacements = { ...offerData.customData };
        }
        
        // Base Replacements Map
        const replacements = {
          ...
          '{{offer_ref_no}}': offerData?.refNo || `OFFER/${new Date().getFullYear()}/${String(applicant?._id || '').slice(-4).toUpperCase()}`,
